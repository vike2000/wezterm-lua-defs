...
fn is_mlua_multivalue(ty: &syn::Type) -> bool { //cspell:ignore multivalue
	match ty {
		syn::Type::Path(tp) => {
			tp.path.segments.last().is_some_and(|seg| seg.ident == "MultiValue")
		}
		_ => false,
	}
}
...
fn fn_signature_param(args: &Args, function_context: &FunctionContext, visiting_context: &mut VisitingContext, _stats: &mut Stats, ty: Option<syn::Type>, pat: syn::Pat) -> LuaParam {
	LuaParam {
		name: if args.debug.unwrap_or_default() > 0
		...
		match input {
			syn::FnArg::Typed(pat_type) => {
				...
				match pat_type.pat.as_ref() {
					syn::Pat::Tuple(top_tuple) => for p in top_tuple.elems.iter().map(|p| fn_signature_param(args, function_context, visiting_context, stats, None, p.clone())) {
						params.push(p)
					},
					_ => params.push(fn_signature_param(args, function_context, visiting_context, stats, Some(*pat_type.ty.clone()), *pat_type.pat.clone())),
				};
			}
...
fn emit_userdata(name: &str, methods: &[LuaFunction]) -> String {
	...
	for f in methods {
		for p in f.params.iter().filter(|p| p.name != "this") {
			out.push_str(&format!("---@param {} {}\n", p.name, lua_type_to_luals(&p.lua_type)));
		}
